...
export function sanitizeUntrustedProfile(
  ...
  return {
    ...
    fieldOfStudy: scalar(profile.fieldOfStudy),
    currentDegreeLevel: scalar(profile.currentDegreeLevel),
    targetDegreeLevel: scalar(profile.targetDegreeLevel),
    currentInstitution: scalar(profile.currentInstitution),
    graduationYear: scalar(profile.graduationYear),
    nationality: scalar(profile.nationality),
    countryOfResidence: scalar(profile.countryOfResidence),
    preferredStudyCountries: sanitizeUntrustedValues(
      profile.preferredStudyCountries ?? [],
    ),
    intendedStartYear: scalar(profile.intendedStartYear),
    fundingRequirement: scalar(profile.fundingRequirement),
    languageTestStatus: scalar(profile.languageTestStatus),
    workAuthorization: scalar(profile.workAuthorization),
    sponsorshipRequired: profile.sponsorshipRequired,
    availability: scalar(profile.availability),
